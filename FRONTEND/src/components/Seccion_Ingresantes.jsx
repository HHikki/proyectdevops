...
const StudentCard = ({ student, color, idx }) => (
  <div
    className={`rounded-2xl shadow-md ${color.border} border-2 transform transition-transform duration-300 hover:scale-105 hover:shadow-xl bg-opacity-90 group`}
    data-aos="fade-up"
  ...
  >
    ...
    <div className="flex-1 flex flex-col justify-between px-6 py-5 text-center">
      <h3
        className={`font-bold text-lg mb-2 uppercase tracking-wide ${color.name} transition group-hover:underline`}
      >
        {student.nombre}
      </h3>
      ...
      <p className="font-semibold text-base text-gray-700 mb-4 transition">
        {student.carrera}
      </p>
      <div className="flex justify-center gap-3 mt-auto">
        {redes.map((r, i) => (
          <a
            key={i}
            href="#"
            className={`rounded-full w-10 h-10 flex items-center justify-center text-xl text-white shadow transition duration-300 hover:scale-110 ${r.color}`}
