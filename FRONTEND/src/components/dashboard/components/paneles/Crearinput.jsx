...
import { supabase } from "../../../../supas/supabaseClient";
...
export default function Crearinput({
  ...
  async function subirImagen(file) {
    ...
    const { data, error } = await supabase.storage
      .from("mediaprisma") // nombre del bucket
    ...
    // Obtener URL pública - corrección: usar el mismo bucket
    const { data: urlData } = supabase.storage
      .from("mediaprisma") // Usar el mismo bucket, no "imagenes"
  ...
  const handPost = async () => {
    try {
      ...
      const postData = {
        ...
        postType: tipo_pub,
        imagenes: imageUrls.map((url) => ({ image_url: url, is_cover: false })), // Array de URLs de las imágenes
